...
sigident::plot_grid_model_plot(
    model = diagnostic_svm$svm_model,
    filename = filename
)
```
```{r out.width='80%'}
...
sigident::plot_grid_model_plot(
    model = diagnostic_knn$knn_model,
    filename = filename
)
```
```{r out.width='80%'}
...
diagnostic_models <- list(
  "svm" = list(
        "model" = diagnostic_svm$svm_model,
        "confmat" = diagnostic_svm$confmat_svm,
        "prediction" = diagnostic_svm$predicted_svm,
        "auc" = as.numeric(diagnostic_svm$roc_svm$auc)
    ),

    "knn" = list(
      "model" = diagnostic_knn$knn_model,
      "confmat" = diagnostic_knn$confmat_knn,
      "prediction" = diagnostic_knn$predicted_knn,
      "auc" = as.numeric(diagnostic_knn$roc_knn$auc)
    ),

    "rf" = list(
      "model" = diagnostic_rf$rf_model,
      "confmat" = diagnostic_rf$confmat_rf,
      "prediction" = diagnostic_rf$predicted_rf,
      "auc" = as.numeric(diagnostic_rf$roc_rf$auc)
    )
)
```
