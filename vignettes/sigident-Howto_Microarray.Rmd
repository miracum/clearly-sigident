...
```
```{r}
# TODO only for my development #JV
# loading mergeset exported after usage of mergeGEO
mergeset <- readRDS("/Users/johannesvey/Bioinfo/Pipeline/RMetaSigDP/mergegeo/mergeset.rds")
samplemetadata <- read.csv("/Users/johannesvey/Bioinfo/Pipeline/RMetaSigDP/sigident/inst/demofiles/lungcancer_sample_metadata.csv")
studymetadata <- read.csv("/Users/johannesvey/Bioinfo/Pipeline/RMetaSigDP/sigident/inst/demofiles/lungcancer_study_metadata.csv")
```
...
```{r results='hide', message=FALSE, warning=FALSE, error=FALSE}
dd <- createDiagnosisDesign_(samplemetadata, controlname, targetname, targetcol)
diagnosis <- dd$diagnosis
design <- dd$design

# TODO make this working generically and implement it into functin createBatch(batchcorrection.R)
study_length <- as.data.frame(table(samplemetadata$study))
batch <- rep(c(1,2,3,4), times = c(study_length[1,2], study_length[2,2], study_length[3,2],                                       study_length[4,2]))

gPCA_after <- batchCorrection_(mergeset, batch)
