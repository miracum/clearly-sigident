...
Setting `type = "elasticnet_grid"` of the function `signature` conducts a grid search calculating a pair of alpha and lambda for the 'glmnet'-method, that minimizes the CV error, using the R package [caret](https://cran.r-project.org/web/packages/caret/index.html).  
...
diagnostic_models <- list(
  ...
  "elasticnet_grid" = list(
    ...
    "prediction" = diagnostic_glmgrid$predicted_elasticnet,
...
```{r}
diagnostic_models$elasticnet_grid$confmat
```
...
signaturegenes <- sigident::gene_map_sig(
  mergeset = mergeset,
  model = diagnostic_models$elasticnet_grid$model
)
data.table::fwrite(signaturegenes,
                   paste0(csvdir, "signaturegenes_elasticnet_grid.csv"))
```
...
```{r}
validation_results[["GSE30219"]]$elasticnet_grid$confmat
```

```{r results='hide', message=FALSE, warning=FALSE, error=FALSE}
# create roc plot
filename <- paste0(plotdir, "ROC_validation_GSE30219_elasticnet_grid.png")
sigident::plot_rocplot(roc = validation_results[["GSE30219"]]$elasticnet_grid$roc,
                       filename = filename)
```
```{r out.width='80%'}
knitr::include_graphics(filename)
...
```{r}
validation_results[["GSE33356"]]$elasticnet_grid$confmat
```

```{r results='hide', message=FALSE, warning=FALSE, error=FALSE}
# create roc plot
filename <- paste0(plotdir, "ROC_validation_GSE33356_elasticnet_grid.png")
sigident::plot_rocplot(roc = validation_results[["GSE33356"]]$elasticnet_grid$roc,
                       filename = filename)
```
```{r out.width='80%'}
knitr::include_graphics(filename)
...
```{r}
validation_results[["GSE102287"]]$elasticnet_grid$confmat
```

```{r results='hide', message=FALSE, warning=FALSE, error=FALSE}
# create roc plot
filename <- paste0(plotdir, "ROC_validation_GSE102287_elasticnet_grid.png")
sigident::plot_rocplot(roc = validation_results[["GSE102287"]]$elasticnet_grid$roc,
                       filename = filename)
```
```{r out.width='80%'}
knitr::include_graphics(filename)
