...
```{r}
# workaround
load("../../../Shiny/RMetaSigDP_privVey/inst/application/data/esets.RData")

GSE18842_meta <- data.table::data.table(
  cbind(
    study = "GSE18842",
    sample = eset1b@phenoData@data[,"geo_accession"],
    target = as.character(eset1b@phenoData@data[,"Tumor"])
  )
)
...
GSE19804_meta <- data.table::data.table(
  cbind(
    ...
    target = as.character(eset2b@phenoData@data[,"Tumor"])
  )
)
...
GSE19188_meta <- data.table::data.table(
  cbind(
    ...
    target = as.character(eset3b@phenoData@data[,"Tumor"])
  )
)
...
table(mergedset@phenoData@data$target)
...
rm("DF", "mergedset", "GSE18842_meta", "GSE19188_meta", "GSE19804_meta")
```
